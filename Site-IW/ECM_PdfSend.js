...
function getCategoryData(jobIdentifier) {
  ...
  categoryData[mapping['Contract_Number']] = jobCode //job.asset.identifier;
...
if (Geopal.Input.isFileUploadSuccessful('file')) {
  ...
  var jobCode = Geopal.Input.request('job_Code');
  logger('jobCode',jobCode)
...
//Geopal.Log.setExtraField4('params ' + JSON.stringify(params));
  ...
  var response = Geopal.HttpClient.postFiles(
    SENDFILE_URL,
    params,
    fileUploads,
    headers
  ...
  //Geopal.Log.setExtraField5('params ' + JSON.stringify(response));
  ...
  if (!body.hasOwnProperty('error')) {
    ...
    logger('categoryData',categoryData)
...
function logger(text, varName){
  ...
  logTxt ? logTxt += '\n' + text +': ' + varName:
  logTxt += text +': ' + varName;
  Geopal.Log.setExtraField5(logTxt);
  }
